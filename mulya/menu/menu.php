<?php
	...
	$aksi = 'mulya/menu/act_menu.php';

	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				$act = "$aksi?mulya=menu&act=edit";
				...
				{
					header("location:m.php?mulya=menu");
				}

			}
			else
			{
				$act = "$aksi?mulya=menu&act=simpan";
			}
		...
		default :
				...
				<tr>
					<td>
						<form class='w3-tiny' action='' method='GET'>	
							<input type='hidden' name='mulya' value='menu'>
							<div class='w3-row'>
								<div class='w3-col s1'>
					...
					<td align='right'><a href='m.php?mulya=menu' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='m.php?mulya=menu&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
			...
			<table class='w3-table w3-striped w3-bordered w3-small w3-hoverable tbl'>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysql_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td>$m[posisi]</td>
							<td>
							<a href='m.php?mulya=menu&act=form&id=$m[id_menu]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a> 
							<a href='$aksi?mulya=menu&act=hapus&id=$m[id_menu]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a> 
				...
				/*<a href='javascript:void(0);' class='detail' onclick=\"PopupCenter('popup/popup.php?mulya=menu&id=$m[id_menu]', 'Edit Setoran', '500', '600')\"><i class='fa fa-list-ul w3-large w3-text-blue-grey'></i></a>*/
				echo"</tbody>
