<?php
	...
	$aksi = 'mulya/modul/act_modul.php';

	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				$act = "$aksi?mulya=modul&act=edit";
				...
				{
					header("location:m.php?mulya=modul");
				}

			}
			else
			{
				$act = "$aksi?mulya=modul&act=simpan";
			}
			...
			echo"<form class='w3-small' method='POST' action='$act'>
				...
				<table>
					...
					<tr>
						...
						<td>:</td>
						<td><input type='text' name='link_menu' class='w3-input' placeholder='link_menu' value='"?><?php echo isset($c['link_menu']) ? $c['link_menu'] : '';?><?php echo"' required>
						</td>
				...
				<tr>
					<td>
						<form class='w3-tiny' action='' method='GET'>	
							<input type='hidden' name='mulya' value='modul'>
							<div class='w3-row'>
								<div class='w3-col s1'>
					...
					<td align='right'><a href='m.php?mulya=modul' class='w3-btn w3-dark-grey w3-small'><i class='fa fa-refresh'></i> Refresh</a>
					<a href='m.php?mulya=modul&act=form' class='w3-btn w3-small w3-blue'><i class='fa fa-file'></i> Tambah</a></td>
				</tr>
				...
				if($fd_kul > 0)
				{
					...
					while ($m = mysql_fetch_assoc($sql_kul)) {
						echo"<tr>
							...
							<td>$m[posisi]</td>
							<td>$m[icon_menu]</td>
							<td><a href='m.php?mulya=modul&act=form&id=$m[id_modul]'><i class='fa fa-pencil-square w3-large w3-text-blue'></i></a> 
							<a href='$aksi?mulya=modul&act=hapus&id=$m[id_modul]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a></td>
