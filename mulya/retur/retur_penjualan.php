<?php
	...
	if(isset($_SESSION['returpj']) AND $_SESSION['returpj'] <> 'TRUE')
	{
	...
	//link buat paging
	$linkaksi = 'm.php?mulya=returpenjualan';
	...
	$aksi = 'mulya/retur/act_retur_penjualan.php';

	switch ($act) {
		default :
			echo"<div class='w3-container w3-small w3-pale-green w3-leftbar w3-border-green'>
				<h4 style='margin-bottom:0;padding-bottom:0;'>Retur Penjualan Barang</h4>
				<p style='margin-top:0;padding-top:0;'><i>Semua Data Retur Penjualan Barang</i></p>
			</div>";
			...
			echo"<div class='w3-row-padding'>
				<div class='w3-col s3 w3-card'>
					...
					<div class='w3-card-2 w3-light-blue'>
						<form action='$aksi?mulya=returpenjualan&act=simpan' method='POST' class='w3-container'>

							<label class='w3-label w3-text-black'>Nomor Transaksi</label>
							<input type='text' name='no_transaksi' id='no_transaksi' placeholder='ketik nomor transaksi ...' class='w3-input w3-tiny w3-border-0' required>
							...
							<p><button class='w3-btn' style='width:100%;' onclick=\"return confirm('Klik OK untuk melanjutkan');\"><i class='fa fa-save'></i> Simpan Retur</button></p>
						</form>
					</div>
				...
				<div class='w3-col s9'>
						...
						<tr>
							<td>
								<form class='w3-tiny' action='' method='GET'>	
									<input type='hidden' name='mulya' value='returpenjualan'>
									<div class='w3-row'>
										<div class='w3-col s1'>
										...
										<div class='w3-col s2'>
											<select name='field' class='w3-select w3-padding'>
												...
												<option value='a.no_transaksi'>NO. TRANSAKSI</option>
										...
										<div class='w3-col s1'>
											<a href='m.php?mulya=returpenjualan' class='w3-btn w3-dark-grey w3-tiny'><i class='fa fa-refresh'></i> REFRESH</a>
										</div>
									</div>
								</form>
							</td>
					...
					<table class='w3-table w3-striped w3-bordered w3-tiny w3-hoverable tbl'>
						...
						$query = "SELECT a.*, b.nama_barang FROM tb_retur_penjualan a LEFT JOIN tb_barang b 
								ON a.kode_barang = b.kode_barang";

						$q 	= "SELECT a.*, b.nama_barang FROM tb_retur_penjualan a LEFT JOIN tb_barang b 
								ON a.kode_barang = b.kode_barang";
						...
						if($fd_kul > 0)
						{
							...
							while ($m = mysql_fetch_assoc($sql_kul)) {
								...
								echo"<tr>
									<td>$no</td>
									<td><a class='w3-hover-text-red w3-text-blue' href='m.php?mulya=penjualan&act=printout&id=$m[no_transaksi]'>$m[no_transaksi]</a></td>
									<td>$m[kode_barang]</td>
									<td>$m[nama_barang]</td>
									<td>Rp. ".number_format($m['harga'])." X $m[qty]</td>
									<td>".number_format($m['disc'])."%</td>
									<td>Rp. ".number_format($total_harga)."</td>
									<td><a href='$aksi?mulya=returpenjualan&act=hapus&id=$m[no_transaksi]&kode=$m[kode_barang]' onclick=\"return confirm('Yakin hapus data');\"><i class='fa fa-trash w3-large w3-text-red'></i></a>
									</td>
								</tr>";
								$no++;
							}
						...
						{
							echo"<tr>
								<td colspan='8'><div class='w3-center'><i>Data Barang Not Found.</i></div></td>
							</tr>";
						}
